...
void lst_insert_sorted(LIST *l, ElemType x) {
	...
	while(current != NULL){ //if there is more than one on the list
		//case if it's at the beginning
		if(x <= l->front->val){
			l->front = temp;
			temp->next = current;
			(l->count)++;
		...
		//case if it's somewhere in the middle
...
/* 	lst_print(a);
	lst_print(b);
	printf("\nAsanity1: %d\nAsanity2: %d\n", lst_sanity1(a), lst_sanity2(a));
	printf("Bsanity1: %d\nBsanity2: %d\n", lst_sanity1(b), lst_sanity2(b)); */
	return;
}
...
/* 	NODE *aTemp = lst->front;
	NODE *aPrevious = lst->front;
	NODE *aNext = lst->front;
	NODE *bTemp = lst->front;
	int foundFront = 0; */
	printf("Cutoff: %d\n", cutoff);
	/*
	while(aTemp != NULL){
	...
	*/
	...
	temp = lst->front;
	printf("MAXVALUE: %d\nCUTOFF: %d\n\n", maxVal, cutoff);
	//cut off is larger than all values
	if(cutoff >= maxVal){
		while(temp != NULL){
			lst_push_back(ret, temp->val);
			lst_pop_front(lst);
	...
	//cut off is less than the max value
	else{
		while(temp != NULL){
			if(temp->val <= cutoff){
				lst_push_back(ret, temp->val);
